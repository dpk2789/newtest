$(function () {
    $("a[data-modal=storeitems]").on("click", function () {
        $("#storeitemsModalContent").load(this.href, function () {
            $("#storeitemsModal").modal({ keyboard: true }, "show");

            $("#storeitems").submit(function () {
                if ($("#storeitems").valid()) {
                    $.ajax({
                        url: this.action,
                        type: this.method,
